...
import { MailService } from 'src/services/email/mail.service';
  ...
  export class ApplicationController {
    ...
    async submitApplication(
      ...
      await this.mailService.sendConfirmationEmail(candidateEmail, candidateName, job.title);
    ...
    async updateStatus(@Param('id') id: string, @Body('status') status: Status){
      ...
      const updatedApplication = await this.applicationService.updateApplicationStatus(numericId, status);
      ...
      switch (status) {
        ...
        case Status.REJECTED:
          statusMessage = 'Votre candidature a été rejetée. Nous vous encourageons à postuler à nouveau à l\'avenir.';
          break;
        case Status.PENDING:
      ...
      await this.mailService.sendMail(candidateEmail, subject, {
    ...
    @UseGuards(JwtAuthGuard, RolesGuard)
    @Roles(Role.RECRUTEUR)
    async getCandidateForJob(@Param('jobId') jobId: string, @Param('candidateId') candidateId: string) {
      const numericId = parseInt(jobId, 10);
      if (isNaN(numericId)) {
        throw new BadRequestException('Invalid ID format');
      }
      ...
      const applications = await this.applicationService.getApplicationsForJob(numericId);
